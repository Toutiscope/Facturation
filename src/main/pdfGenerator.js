...
const { loadConfig } = require('./fileManager')
 ...
 */
async function generatePDF(type, document) {
  try {
...
function renderCustomer(doc, customer) {
  ...
  doc.text(customer.address, margin, startY + (customer.companyId ? 65 : 50))
    .text(`${customer.postalCode} ${customer.city}`, margin, startY + (customer.companyId ? 80 : 65))
...
function renderServices(doc, services) {
  ...
  // Colonnes du tableau
  const col1Width = tableWidth * 0.40  // Description
  ...
  doc.fillColor('#1e293b')
    ...
    .text('Unité', margin + col1Width + col2Width + 5, y + 8, { width: col3Width - 10 })
    .text('P.U. HT', margin + col1Width + col2Width + col3Width + 5, y + 8, { width: col4Width - 10 })
    .text('Total HT', margin + col1Width + col2Width + col3Width + col4Width + 5, y + 8, { width: col5Width - 10 })
  ...
  services.forEach((service, index) => {
    ...
    doc.fillColor('#1e293b')
      .text(service.description, margin + 5, y + 8, { width: col1Width - 10, height: rowHeight - 16 })
      .text(service.quantity.toString(), margin + col1Width + 5, y + 8, { width: col2Width - 10 })
      .text(service.unit, margin + col1Width + col2Width + 5, y + 8, { width: col3Width - 10 })
      .text(`${service.unitPriceHT.toFixed(2)} €`, margin + col1Width + col2Width + col3Width + 5, y + 8, { width: col4Width - 10 })
      .text(`${service.totalHT.toFixed(2)} €`, margin + col1Width + col2Width + col3Width + col4Width + 5, y + 8, { width: col5Width - 10 })
...
function renderTotals(doc, totals) {
  ...
  doc.fontSize(10)
    ...
    .text(`${totals.totalHT.toFixed(2)} €`, rightX + 120, startY, { width: 80, align: 'right' })

  doc.text('TVA (0%):', rightX, startY + 20, { width: 120, align: 'left' })
    .text(`${totals.VAT.toFixed(2)} €`, rightX + 120, startY + 20, { width: 80, align: 'right' })

  // Total TTC en gras
  doc.fontSize(12)
    .font('Helvetica-Bold')
    .text('Total TTC:', rightX, startY + 45, { width: 120, align: 'left' })
    .text(`${totals.totalTTC.toFixed(2)} €`, rightX + 120, startY + 45, { width: 80, align: 'right' })
...
function renderFooter(doc, config, type, document) {
  ...
  if (config.rib.iban) {
    ...
    if (config.rib.bank) {
      doc.text(`Banque: ${config.rib.bank}`, margin, doc.y + (config.rib.bic ? 36 : 24))
    }
  }
