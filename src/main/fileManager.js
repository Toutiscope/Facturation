...
async function loadConfig() {
  try {
    ...
    const data = await fs.readFile(pathsModule.CONFIG_PATH, 'utf-8')
...
async function saveConfig(config) {
  try {
    await fs.writeFile(pathsModule.CONFIG_PATH, JSON.stringify(config, null, 2), 'utf-8')
 ...
 */
async function loadDocuments(type, filters = {}) {
  try {
    const { year, status, search } = filters
    const targetYear = year || new Date().getFullYear()
    const yearFolder = pathsModule.getYearFolder(type, targetYear)
...
async function loadDocument(type, id) {
  try {
    ...
    const yearFolder = pathsModule.getYearFolder(type, year)
 ...
 */
async function saveDocument(type, document) {
  try {
